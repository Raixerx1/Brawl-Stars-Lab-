...
function explicitRelation(candidate: Brawler, target: Brawler) {
  ...
  if (positive) {
    score += candidateListsTarget && targetListsCandidate ? 34 : 27;
    ...
    pushReason(reasons, `${candidate.name} está registrado como counter directo de ${target.name}`);
  }

  if (negative) {
    score -= targetListsCandidateAsGood && candidateListsTargetAsThreat ? 38 : 31;
    pushReason(reasons, `${target.name} también dispone de una interacción favorable contra ${candidate.name}`);
  }
...
function mechanicScore(candidate: Brawler, target: Brawler) {
  ...
  if (target.role === "Tirador" || hasTag(target, "sniper", "tirador", "open")) {
    ...
    if (isThrower(candidate) && targetProfile?.wallBreak && targetProfile.wallBreak < 45) {
      score += 5;
  ...
  if (["Control", "Apoyo"].includes(target.role)) {
  ...
  if (targetWallReliance >= 65 && hasWallbreak(candidate)) {
    ...
    score += edge;
    if (edge >= 6) {
      pushReason(reasons, `${candidate.name} reduce el valor de la cobertura que necesita ${target.name}`);
    }
  }
...
export function evaluateSpecificMatchup(candidate: Brawler, target: Brawler): SpecificMatchup {
  ...
  mechanics.reasons.forEach((reason) => pushReason(reasons, reason));

  const candidateRisk = candidate.firstPickProfile?.counterRisk ?? 50;
  const viabilityAdjustment = (tierBonus[candidate.tier] || 0) - Math.max(0, candidateRisk - 70) * .04;
  const score = clamp(50 + explicit.score + mechanics.score + viabilityAdjustment);

  const confidence: CounterConfidence =
    explicit.reviewed || (explicit.positive && !explicit.negative) ? "Alta" :
    explicit.positive || explicit.negative || mechanics.reasons.length >= 2 ? "Media" :
    "Baja";
