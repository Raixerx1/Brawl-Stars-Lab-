  ...
  return <div className="live-review-v8 live-review-v9 live-review-v10">
    ...
    <section className="panel live-setup-v8">
      <div className="section-title">
        <div><span className="eyebrow">Auto Review v0.16</span><h2>Preparar sesión</h2></div>
    ...
    <section className="live-review-grid-v8">
      <article className="panel live-summary-panel">
        ...
        <div className={`auto-scorecard-v16 verdict-${summary.scorecard.verdict.toLowerCase().replace("í", "i").replace("ó", "o").replace(" ", "-")}`}>
          <div className="auto-score-overall-v16">
            <span>Nota de ejecución</span>
            <strong>{summary.scorecard.verdict === "Sin datos" ? "—" : summary.scorecard.overall}</strong>
            <b>{summary.scorecard.verdict}</b>
            <small>{summary.scorecard.reviewCoverage}% de detecciones revisadas</small>
          </div>
          <div className="auto-score-metrics-v16">
            {[
              ["Posición", summary.scorecard.positioning],
              ["Recursos", summary.scorecard.resources],
              ["Objetivo", summary.scorecard.objective],
              ["Tempo", summary.scorecard.tempo],
            ].map(([label, value]) => <span key={label}>
              <b>{summary.scorecard.verdict === "Sin datos" ? "—" : value}</b>
              <small>{label}</small>
              <i><em style={{ width: `${value}%` }} /></i>
            </span>)}
          </div>
          <div className={`auto-key-moment-v16 impact-${summary.scorecard.keyMoment?.impact.toLowerCase() || "neutral"}`}>
            <span>Momento clave</span>
            {summary.scorecard.keyMoment ? <>
              <b>{formatLiveTime(summary.scorecard.keyMoment.second)} · {summary.scorecard.keyMoment.label}</b>
              <small>{summary.scorecard.keyMoment.reason}</small>
            </> : <small>Añade o confirma eventos para localizar la jugada más influyente.</small>}
          </div>
        </div>
